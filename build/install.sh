#!/bin/sh
# chapterize installer
#   curl -fsSL https://github.com/MrDwarf7/chapterize.rs/raw/main/build/install.sh | sh
#
# Environment variables:
#   CHAPTERIZE_VERSION  - version tag to install (default: latest)
#   CHAPTERIZE_INSTALL  - install dir (default: /usr/local/bin or ~/.local/bin)
#   GITHUB_TOKEN        - avoids API rate limits in CI

set -e

REPO="MrDwarf7/chapterize.rs"
RELEASES_URL="https://github.com/$REPO/releases"
INSTALL_DIR="${CHAPTERIZE_INSTALL:-}"
...
fetch_latest_version() {
  auth_arg=""
  [ -n "${GITHUB_TOKEN:-}" ] && auth_arg="-H \"Authorization: Bearer $GITHUB_TOKEN\""
  ...
  [ -n "$tag" ] || die "could not resolve latest release. Install via \`cargo install chapterize\` instead."
  echo "$tag"
}
...
version="${CHAPTERIZE_VERSION:-latest}"
...
# CI artifact name: chapterize-<target>-<tag>.zip
artifact="chapterize-${target}-${version}.zip"
...
binary="chapterize"
exe=""
[ "$os" = "macos" ] && os_check="" # no extension on macOS
# Windows already filtered out above, but just in case:
[ "$os" = "windows" ] && exe=".exe"

src=$(find "$tmpdir/extracted" -type f -name "${binary}${exe}" 2>/dev/null | head -1)
[ -n "$src" ] || die "binary '${binary}${exe}' not found in release archive"

mv "$src" "$INSTALL_DIR/$binary$exe"
chmod +x "$INSTALL_DIR/$binary$exe"
log "installed to $INSTALL_DIR/$binary$exe"
...
echo ""
echo "  chapterize $ver installed. Run \`chapterize --help\` to get started."
